...
sqlCreatePost     text "insert into Posts(threadID, userID, postTime, format, content) values (?1, ?2, strftime('%s','now'), ?3, ?4) returning id"
sqlSavePost       text "update Posts set content = ?4, format = ?3, editUserID = ?2, editTime = strftime('%s','now') where id = ?1 returning id"

sqlCreateThread   text "insert into Threads(userID) values (?1) returning id"
...
.do_quote:
        ...
        mov     edx, [esi+TSpecialParams.userStatus]
        and     edx, permMultiQuote
        stdcall StrCatPost, [.source], [.format], edx, eax ; length from the stack.
...
; create new thread.
        ...
        cinvoke sqliteStep, [.stmt]
        cmp     eax, SQLITE_ROW ; returning statement!
        jne     .error_write
...
.update_or_insert:
        ...
        mov     ecx, [.postID]
        test    ecx, ecx
        cmovz   ecx, [.threadID]        ; if this is insert new post query.
        ...
        cinvoke sqliteStep, [.stmt]
        cmp     eax, SQLITE_ROW         ; insert and update with returning clause!
        jne     .error_write
...
; Concatenates the source from [.pSource] to the [.hString],
; optionally stripping from the source all the quotes.
;
; .hString - destination string.
; [.format] - 0: minimag, 1:bbcode
; .fStrip - boolean.
; .pSource - pointer to the source code string.
; .Length - the length of the source code string in bytes.
;
;
proc StrCatPost, .hString, .format, .fMultiquote, .pSource, .Length
begin
        cmp     [.fMultiquote], 0
        je      .strip

.all:
        stdcall StrCatMem, [.hString], [.pSource], [.Length]
        return
...
.minimag:
        stdcall __StrCatMinimag, [.hString], [.pSource], [.Length]
        return
        
.bbcode:
        stdcall __StrCatBBCode, [.hString], [.pSource], [.Length]
        return
endp


proc __StrCatMinimag, .hString, .pSource, .Length
.maxNesting = $100000
begin
        pushad
        ...
        mov     edi, eax
        add     edi, [eax+string.len]

        mov     ecx, [.Length]

        xor     ebx, ebx
        xor     edx, edx
        inc     edx             ; new line assumed from the start.
...
.eol_found:                       ; notice, that here EDX == 1!
        xor     al, $0d xor $0a
        cmp     [esi], al
        jne     .loop
...
.nor_end:
        cmp     dword [esi], ';---'
        je      .srch_eol       ; it is a comment and there is no closing tag.

        cmp     byte [esi], ' '
        jbe     .srch_eol      ; comment
...
proc __StrCatBBCode, .hString, .pSource, .Length
begin
        pushad
...
.not_quote:
        test    ebx, ebx
        jz      .store  ; ignore the not paired closing tags.
        ...
        sub     edx, esi        ; skip to the end of the tag.
        sub     ecx, edx
        add     esi, edx
        jmp     .loop
