...
sqlCheckBots  text "select remoteIP, count() as cnt, ((max(time) - min(time))/count()) as avgTime from UserLog where Activity = 3 and remoteIP = (select ip_from from WaitingActivation where a_secret = ?1)"
sqlDeleteBots text "delete from WaitingActivation where ip_from = ?1"
...
; Bot registration protection:
        ...
        lea     eax, [.stmt]
        cinvoke sqlitePrepare_v2, [hMainDatabase], sqlCheckBots, sqlCheckBots.length, eax, 0

        stdcall StrPtr, ebx
        cinvoke sqliteBindText, [.stmt], 1, eax, [eax+string.len], SQLITE_STATIC
        cinvoke sqliteStep, [.stmt]

        cinvoke sqliteColumnInt, [.stmt], 0     ; IP address
        mov     [.ip_addr], eax

        cinvoke sqliteColumnInt, [.stmt], 2  ; avgTime
        push    eax
        
        cinvoke sqliteColumnInt, [.stmt], 1      ; count
        push    eax
        ...
        test    eax, eax
        jz      .bot_detected        ; Strange, must be at least 1. Assume bots!
        ...
        cmp     edx, MIN_TIME_BETWEEN_REGISTRATIONS     ; hard coded?
        jae     .bot_not_detected

; delete all bots from WaitingActivation

.bot_detected:
        lea     eax, [.stmt]
        cinvoke sqlitePrepare_v2, [hMainDatabase], sqlDeleteBots, sqlDeleteBots.length, eax, 0
        cinvoke sqliteBindInt, [.stmt], 1, [.ip_addr]
        cinvoke sqliteStep, [.stmt]
        cinvoke sqliteFinalize, [.stmt]
        jmp     .msg_new_account      ; fake message for account OK
