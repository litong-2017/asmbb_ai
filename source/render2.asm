...
PHashTable tableSpecial, tpl_func,                              \
        ...
        "ignored",     RenderTemplate.sp_blacklist,             \
        "?ignored=",   RenderTemplate.sp_isignored,             \
...
.usr_loop:
        ...
        add     edi, 4
        xor     ecx, '<q >' xor '</q>' ; turns "<u >" into "</u>" and vice versa
...
@@:
        ...
        stdcall TextIns, edx, txt '<table border cellspacing=0 cellpadding=4 style="word-break:break-word;"><caption>Attached files:</caption><tr>'

        cmp     [.fEdit], 0
        je      .edit_ok

        stdcall TextIns, edx, txt '<th>Ins</th>'
...
.att_loop:
        ...
        stdcall TextIns, edx, <txt '<td><button class="btn" onclick="insertText(', "'source', '/!attached/">
        stdcall TextIns, edx, [.fileid]
        stdcall TextIns, edx, <txt "')", '">⎀</button>'>
...
.edit_ok2:
        ...
        stdcall StrDel ; from the stack
        stdcall TextIns, edx, txt '</a><td class="filesize">'

        stdcall FormatFileSize, [.filesize]
        stdcall TextIns, edx, eax
        stdcall StrDel, eax

        stdcall TextIns, edx, txt '<td class="filetime">'
        stdcall TextIns, edx, [.uploadtime]
        stdcall TextIns, edx, txt '<td class="filecnt">'
        stdcall TextIns, edx, [.count]

        cmp     [.fEdit], 0
        jne     .checksum_ok

        stdcall TextIns, edx, txt '<td class="checksum">'
...
.sp_blacklist:
        stdcall RenderBlacklist, [.pSpecial]
        jmp     .special_ttext


.sp_isignored:
        call    .get_number
        stdcall GetIgnoredBool, ebx, [.pSpecial]
        jmp     .special_int
...
begin
        stdcall Utf8ToAnsi, [.hString], KOI8_R
        push    eax
...
cDefaultSkin       text "NoCSS"
cDefaultMobileSkin text "NoCSS"
