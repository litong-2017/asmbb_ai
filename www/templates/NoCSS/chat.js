...
function CreateUserSpan(user, original) {
  ...
  if (user != original) {
    c = '<mark title="' + original + '">' + user + '</mark>';
  } else {
    c = user;
  }
  ...
  for (var i = 0; i < msgset.msgs.length; i++) {
    ...
    if ( ! document.getElementById("chat" + msg.id) ) {
      ...
      if ( cdate != fdate ) {
        ...
        date.innerHTML = '<mark><time>'+cdate+'</time></mark><hr size=1>';
        all.appendChild(date);
      }

      var p = document.createElement('p');
      p.id = "chat" + msg.id;
      p.innerHTML = '<p><small><em onclick="InsertNick(this)">' + CreateUserSpan(msg.user, msg.originalname) +
                    '</em> <time>(' + hours + ':' + minutes + ':' + seconds + ')</time></small>: ' +
                    replaceEmoticons(linkify(formatEmoji(msg.text)));
...
function OnUserChanged (e) {
  ...
  if ( (usr.status == 0) || (!(usr.events & 3)) )  {
    ...
    else sys_log.insertAfter(p, sys_log.firstChild);
  }
}
