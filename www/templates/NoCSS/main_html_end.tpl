<hr size=1>
<footer>
<small>
  <p>[case:[special:lang]|
    <a href="/!users_online" title="Recent activities of the visitors.">Online</a> now: [special:visitors]
  |
    <a href="/!users_online" title="Последни действия на посетителите.">Онлайн</a> в момента: [special:visitors]
  |
    <a href="/!users_online" title="Последние действия потребителей.">Сейчас</a> в сети: [special:visitors]
  |
    Actuellement <a href="/!users_online" title="Activités récentes des visiteurs.">en ligne</a>: [special:visitors]
  |
    Jetzt <a href="/!users_online" title="Letzte Aktivitäten der Besucher.">online</a>: [special:visitors]
  ]

  [special:stats]

  <p>[case:[special:lang]|Page processing time|Време за обработка на страницата|Время обработки страницы|Page chargée en|Seite verarbeitet in]: [special:timestamp] ms
</small>
</footer>
<hr size=1>
<div id="credits" align=right>
<small>
  [special:version]; [special:sqliteversion];<br>
  <b>&copy;2016..2024 John Found</b>; Licensed under <b>EUPL</b>; Powered by <b><a href="http://flatassembler.net">Assembly language</a></b> Created with <b><a href="http://fresh.flatassembler.net">Fresh IDE</a></b>
</small>
</div>
[special:environment]

